...
func GenCompressInfo(msgSize int) uint16 {
	...
	var enable uint16
	if config.Parameters.Compression.Enable && msgSize >= int(config.Parameters.Compression.FileSize) {
		enable = 1
	} else {
		enable = 0
	}

	if AlgoType(config.Parameters.Compression.CompressAlgo) > AlgoType(0xFF) {
		panic("compress algo value invalid")
	}

	return (enable << 8) | uint16(config.Parameters.Compression.CompressAlgo)
}
...
func Compress(message []byte) ([]byte, error) {
	switch AlgoType(config.Parameters.Compression.CompressAlgo) {
	case BZIP2:

...
func GetLocalIP() string {
	if config.Parameters.InterfaceName != "" && config.Parameters.InnerIp != "" {
		iface, err := net.InterfaceByName(config.Parameters.InterfaceName)
		if err != nil {
		...
		for _, addr := range addrs {
			if strings.Split(addr.String(), "/")[0] == config.Parameters.InnerIp {
				return config.Parameters.InnerIp
			}
		}
...
func GetPublicIP() string {
	if config.Parameters.PublicIp == "" {
		return GetLocalIP()
	}
	return config.Parameters.PublicIp
}

func GetPortFromParamsByProtocol(protocol string) int {
	switch protocol {
	case "udp":
		return int(config.Parameters.UPort)
	case "kcp":
		return int(config.Parameters.KPort)
	case "quic":
		return int(config.Parameters.QPort)
	case "tcp":
		return int(config.Parameters.TPort)
	default:
...
func GetLogDir() string {
	return config.Parameters.LogDir
}

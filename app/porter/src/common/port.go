...
func RandomPort(protocol string, connectionID string) uint16 {
	...
	if port, ok := PortSet.Cache.Load(key); ok {
		if time.Now().After(port.(*UsingPort).Timestamp.Add(time.Duration(timeout) * time.Second)) {
...
func InitPorts() {
	PortSet = Ports{
		begin:      int(config.Parameters.RandomPortBegin),
		ranges:     int(config.Parameters.RandomPortRange),
		usingPorts: new(sync.Map),
		WriteMutex: new(sync.Mutex),
		Cache:      new(sync.Map),
		PorterDB:   OpenPorterDB(config.Parameters.PorterDbPath),
	}
}

...
func newConnState(listenr net.Conn, addr string) *ConnState {
	return &ConnState{
		conn:         listenr,
		writer:       bufio.NewWriterSize(listenr, int(config.Parameters.WriteBufferSize)),
		messageNonce: 0,
...
func (p *TCPProxyServer) monitorPeerStatus() {
	...
	for {
		...
		case <-interval:
			...
			timeout := config.Parameters.PortTimeout
			if config.Parameters.PortTimeout <= 0 {
				timeout = DEFAULT_PORT_CACHE_TIME
			}
			common.PortSet.Cache.Range(func(key, value interface{}) bool {
				if time.Now().After(value.(*common.UsingPort).Timestamp.Add(time.Duration(timeout) * time.Second)) {

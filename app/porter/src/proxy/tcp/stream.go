...
import (
	...
	"v2ray.com/core/app/porter/src/common"
	"v2ray.com/core/app/porter/src/internal/protobuf"
	"v2ray.com/core/app/porter/src/types/opcode"
...
func (p *TCPProxyServer) receiveTcpRawMessage(state *ConnState, sendTo string) ([]byte, error) {
	...
	//start := time.Now()
	if binary.BigEndian.Uint32(sizeBuf) != config.Parameters.NetworkId {
...
func receiveMessage(state *ConnState) (*protobuf.Message, error) {
	...
	if binary.BigEndian.Uint32(buffer) != config.Parameters.NetworkId {
		return nil, errors.Errorf("networkID is not match the message info which is contained in msg 4 bytes ahead when recvMessage, "+
			"recv.NetID:%d, setting.NetID:%d", binary.BigEndian.Uint32(buffer), config.Parameters.NetworkId)
	}
	...
	if msg.Opcode == 0 || msg.Sender == nil || msg.Sender.NetKey == nil || len(msg.Sender.Address) == 0 || msg.NetID != config.Parameters.NetworkId {
...
func prepareMessage(message proto.Message, state *ConnState) *protobuf.Message {
	...
	if err != nil {
		log.Errorf("in prepareMessage, GetOpcode err:%s", err.Error())
		return nil
	}

	var id common.ID
	if pubkey, err := hex.DecodeString(config.Parameters.PubKey); err == nil {
		id = common.CreateID(common.GetPublicHost("tcp"), pubkey)
	} else {
		log.Errorf("hex.DecodeString(pubkey:%s) err:%s", config.Parameters.PubKey, err.Error())
		return nil
	}
	...
	return &protobuf.Message{
		...
		Message:      bytes,
		Sender:       (*protobuf.ID)(&id),
		MessageNonce: atomic.AddUint64(&state.messageNonce, 1),
		NetID:        config.Parameters.NetworkId,
	}
}
...
func replySyncMessage(state *ConnState, message proto.Message, nonce uint64) error {
	...
	var enable uint16
	if config.Parameters.Compression.Enable {
		enable = 1
	} else {
		enable = 0
	}
	// Serialize size.
	buffer := make([]byte, 10)
	binary.BigEndian.PutUint32(buffer, config.Parameters.NetworkId)
	binary.BigEndian.PutUint16(buffer[4:], uint16(enable<<8)|(uint16(config.Parameters.Compression.CompressAlgo)&0xFF))
